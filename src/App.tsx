...
export default function App() {
  const [activeTab, setActiveTab] = useState<"home" | "bot" | "smarttalk" | "exam" | "profile">("home");
  const [user, setUser] = useState<UserProfile | null>(null);
  const [isGuest, setIsGuest] = useState(false);

  // Remove the automatic default user fetch
  // useEffect(() => {
  //   fetch("/api/profile/default-user")
  //     .then(res => res.json())
  ...
  const handleLogin = (loggedInUser: UserProfile) => {
    ...
    setIsGuest(false);
    // In a real app, we would save a token here
  };
    ...
    <HelmetProvider>
      <div className="min-h-screen bg-[#F5F5F0] text-[#1A1A1A] font-sans">
        ...
        <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-black/5 px-6 py-3 flex justify-between items-center md:hidden z-50">
          ...
          <button onClick={() => setActiveTab("bot")} className={`p-2 ${activeTab === "bot" ? "text-emerald-600" : "text-gray-400"}`}>
            <BotIcon />
          </button>
...
function BotIcon() { return <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 8V4H8"/><rect width="16" height="12" x="4" y="8" rx="2"/><path d="M2 14h2"/><path d="M20 14h2"/><path d="M15 13v2"/><path d="M9 13v2"/></svg>; }
